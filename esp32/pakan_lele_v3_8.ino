...
#include <RTClib.h>
#include <WiFi.h>
#include <WebSocketsClient.h>   // arduinoWebSockets by Markus Sattler (Links2004)
#include <MQTTPubSubClient.h>   // by hideakitai (MQTT over WebSocket)
...
const char* WIFI_SSID     = "wifi-lele";
const char* WIFI_PASSWORD = "polman2026";

const char* MQTT_SERVER   = "mqtt.trin-polman.id";  // domain Cloudflare (WSS), BUKAN IP lokal
const uint16_t MQTT_PORT  = 443;                    // WSS lewat Cloudflare = 443
const char* MQTT_PATH     = "/";                    // mosquitto WebSocket di root "/"
...
WebSocketsClient wsClient;       // transport WebSocket Secure (TLS via Cloudflare)
MQTTPubSubClient mqttClient;     // MQTT over WebSocket
...
// PWM spinner DIBUAT KONSTAN 255 untuk SEMUA batch (tidak lagi ganjil/genap
// seperti versi sebelumnya). SPINNER_PWM_MIN sengaja DISAMAKAN dengan
// SPINNER_PWM_MAX, bukan dihapus, karena spinnerPWMForDispense() (fungsi
// referensi ramp-down di bawah, sudah tidak dipakai di dispense loop) masih
// mengacu ke kedua konstanta ini.
// AKIBAT DESAIN YANG PERLU DISADARI: efek sebaran lama "batch genap jatuh
// lebih dekat ke pusat kolam, batch ganjil ke tepian" HILANG - semua batch
// kini melempar pada kecepatan penuh yang sama, jadi jarak lemparnya sama
// untuk semua batch. Kalau pakan masih perlu tersebar sampai ke tengah
// kolam, itu harus dicapai lewat mekanisme lain (mis. sudut piring lempar),
...
int     spinnerState      = 0;   // 0=stop, 1=CW, 2=CFCW
...
float samplingCalFactor = -1762;

// =====================================================
// REGRESSION CALIBRATION (LINEAR CORRECTION)
// =====================================================
...
// =====================================================
// INPUT MANUAL RATA-RATA BERAT IKAN
// =====================================================
...
// =====================================================
// INPUT JUMLAH IKAN (kelipatan bertingkat)
// =====================================================
...
// sering muncul (mode/modus histogram resolusi 0.1g).
// Jika ada seri (beberapa nilai sama-sama paling sering),
// diambil rata-rata nilai yang seri tersebut.
const unsigned long SAMPLING_CAPTURE_MS   = 5000UL; // durasi capture
const int           SAMPLING_MIN_READINGS = 5;       // min readings valid
const int           SAMPLING_HIST_MAX_G   = 801;     // rentang histogram 0-800g
const int           SAMPLING_HIST_RES     = 10;      // resolusi: 10 = per 0.1g
const int           SAMPLING_HIST_MAX_BIN = SAMPLING_HIST_MAX_G * SAMPLING_HIST_RES; // total bin (tenths gram)
...
// =====================================================
// ANTI-CLOG SERVO (saat dispensing pakan menyangkut di pintu)
// =====================================================
// Selama dispensing, berat chamber dipantau tiap ANTICLOG_CHECK_INTERVAL_MS.
// Jika dalam jendela waktu itu berat TIDAK berkurang minimal
// ANTICLOG_MIN_DROP_G (indikasi pakan menyangkut/nge-jam di pintu
// trapdoor), maka servo akan MENUTUP sesaat lalu MEMBUKA kembali
// untuk melepaskan sumbatan, tanpa mematikan spinner.
// Mekanisme ini TIDAK dijalankan bila chamber sudah nyaris kosong
// (<= EMPTY_THRESHOLD_G) -> proses berjalan seperti biasa sampai selesai.
const unsigned long ANTICLOG_CHECK_INTERVAL_MS = 4000;  // jendela pantau berat chamber
const float         ANTICLOG_MIN_DROP_G        = 2.0;   // minimal penurunan berat = dianggap lancar
const unsigned long ANTICLOG_CLOSE_MS          = 500;   // durasi pintu menutup saat unclog
const unsigned long ANTICLOG_REOPEN_SETTLE_MS  = 300;   // jeda setelah pintu dibuka kembali
...
void servoOpen();
void servoClose();
bool performAntiClogCycle(int batchNo, int totalBatches);
...
// Pembulatan 1 desimal yang akurat untuk nilai umum (avg, biomassa, dst),
String fmt1(float v) {
  bool neg = v < 0;
  if (neg) v = -v;
  long tenths = (long)(v * 10.0 + 0.5);   // pembulatan ke 1 desimal
  int  whole  = tenths / 10;
  int  dec    = tenths % 10;
  return String(neg ? "-" : "") + String(whole) + "." + String(dec);
}

// Khusus tampilan LIVE saat sampling: sama seperti formatGram() tapi
// menyertakan 1 angka di belakang koma, supaya perubahan kecil berat
// ikan di timbangan sampling terlihat lebih presisi di LCD.
String formatGram1(float v) {
  if (v < 0)      v = 0;
  if (v > 9999.9) v = 9999.9;
  long tenths = (long)(v * 10.0 + 0.5);   // pembulatan ke 1 desimal
  int  whole  = tenths / 10;
  int  dec    = tenths % 10;
  String ws = String(whole);
  while (ws.length() < 3) ws = "0" + ws;  // padding biar rapi di LCD
  return ws + "." + String(dec);
}
...
// =====================================================
bool mqttReady() {
  return MQTT_ENABLE && WiFi.status() == WL_CONNECTED && mqttClient.isConnected();
}
...
void setupWiFi() {
  ...
  if (MQTT_ENABLE) {
    ...
    // Argumen ke-5 "mqtt" = WebSocket subprotocol (WAJIB; default "arduino" ditolak mosquitto).
    wsClient.beginSSL(MQTT_SERVER, MQTT_PORT, MQTT_PATH, "", "mqtt");
    wsClient.setReconnectInterval(3000);
    mqttClient.begin(wsClient);
    mqttClient.setKeepAliveTimeout(60);   // PING < 100s agar Cloudflare tak putus koneksi idle
  }
...
void reconnectMqttIfNeeded() {
  ...
  if (connected) {
    // Subscribe per-topik dgn callback → diarahkan ke onMqttMessage.
    // topicCommand/topicConfig adalah variabel global (non-capturing lambda).
    mqttClient.subscribe(topicCommand, [](const String& payload, const size_t size) {
      onMqttMessage(topicCommand, payload, size);
    });
...
void maintainNetwork() {
  ...
  if (MQTT_ENABLE) {
    ...
    // Heartbeat status (throttled): saat feeding, loop() utama terblokir di
    // runFeedingSession sehingga status tak terkirim > 30s → dashboard salah
    // menandai OFFLINE. maintainNetwork() dipanggil di semua loop feeding, jadi
...
void onMqttMessage(const String& topic, const String& payload, const size_t size) {
  ...
  if (topicStr == topicCommand) {
    ...
    if (strcmp(command, "manual_feed_adaptive") == 0) {
      ...
      if (feedingInProgress) { publishAck(command, false, "Sedang feeding"); return; }
      // PENTING: JANGAN pakai `doc["target_g"] | 0.0` — di ArduinoJson 6 operator
      // `|` bergantung pada is<double>() yang TIDAK reliabel utk nilai JSON integer
      // (backend kirim target_g:100, bukan 100.0) → bisa jatuh ke 0.0 → ditolak
      // diam-diam. Pakai .as<float>() (konversi int/float aman) + dukung string.
      JsonVariant tgv = doc["target_g"];
      float g = tgv.is<const char*>() ? atof(tgv.as<const char*>()) : tgv.as<float>();
      Serial.printf("[CMD] manual_feed_gram target_g=%.1f\n", g);
      if (g < 10 || g > 5000) {
        publishAck(command, false, "Range 10-5000g (baca=" + String(g, 1) + ")"); return;
      }
...
bool captureFishWeightMode(float &resultGram) {
  static uint8_t hist[SAMPLING_HIST_MAX_BIN]; // static: alokasi di data segment, bukan stack
  memset(hist, 0, sizeof(hist));

  const int minBin = (int)(MIN_FISH_SAVE_G * SAMPLING_HIST_RES);
  ...
  while (millis() - startMs < SAMPLING_CAPTURE_MS) {
    ...
    if (millis() - lastLcdMs >= 200) {
      ...
      int modeNowBin = 0, maxNow = 0;
      for (int b = minBin; b < SAMPLING_HIST_MAX_BIN; b++) {
      ...
      String line1 = (maxNow > 0)
        ? (formatGram1((float)modeNowBin / SAMPLING_HIST_RES) + "g  " + String(remainMs / 1000 + 1) + "s  ")
    ...
    if (scaleSampling.is_ready()) {
      ...
      if (raw >= MIN_FISH_SAVE_G && raw <= (float)(SAMPLING_HIST_MAX_G - 1)) {
        int b = (int)roundf(raw * SAMPLING_HIST_RES);
        if (b >= SAMPLING_HIST_MAX_BIN) b = SAMPLING_HIST_MAX_BIN - 1;
  ...
  int   cntTie = 0;
  for (int b = minBin; b < SAMPLING_HIST_MAX_BIN; b++) {
    if (hist[b] == maxCount) { sumTie += ((float)b / SAMPLING_HIST_RES); cntTie++; }
  }
...
// ============================================================
// FEEDING RATE - SUMBER: Jurnal "PENERAPAN TEKNOLOGI IoT
// TERHADAP..." Tabel 2. Perkembangan Ikan Lele.
//
//   Hari 1-7   (2.5-5 g)  : 4% -> 3% dari berat badan
//   Hari 8-14  (5-10 g)   : 3% -> 2% dari berat badan
//   Hari 15-22 (10-20 g)  : 2% dari berat badan (flat)
//
// ============================================================
...
// =====================================================
// Dipanggil saat dispensing terdeteksi macet (berat chamber tidak
// berkurang signifikan dalam ANTICLOG_CHECK_INTERVAL_MS). Spinner
// TETAP berputar, hanya pintu servo yang menutup lalu membuka lagi
// untuk mengguncang/melepas pakan yang menyangkut di pintu.
// Return false jika user menekan BACK (abort) selama siklus ini.
bool performAntiClogCycle(int batchNo, int totalBatches) {
  lcd.clear();
  lcdLine(0, "ANTI-CLOG B:" + String(batchNo) + "/" + String(totalBatches));
  lcdLine(1, "Pintu tutup...");
  servoClose();

  unsigned long t0 = millis();
  while (millis() - t0 < ANTICLOG_CLOSE_MS) {
    maintainNetwork();
    if (backPressed()) return false;
    delay(20);
  }

  lcdLine(1, "Pintu buka lagi");
  servoOpen();

  t0 = millis();
  while (millis() - t0 < ANTICLOG_REOPEN_SETTLE_MS) {
    maintainNetwork();
    if (backPressed()) return false;
    delay(20);
  }
...
// =====================================================
// SPINNER PWM RAMP-DOWN (REFERENSI - TIDAK DIPAKAI)
// =====================================================
uint8_t spinnerPWMForDispense(float chamberGramCurrent, float batchTargetGram) {
...
bool confirmStartFeeding(String title, float totalGram) {
  lcd.clear(); lcdLine(0, title); lcdLine(1, "Total:" + fmt1(totalGram) + "g"); delay(1200);
...
bool runSingleBatch(float targetGram, int batchIndex, int batchNo, int totalBatches, float &actualGram) {
  ...
  lcd.clear(); lcdLine(0,"TARGET REACHED"); lcdLine(1,"CH:" + fmt1(finalGram) + "g"); delay(1000);

  // ---- SPINNER PRE-START (PWM konstan 255 di semua batch) ----
  // PWM TIDAK lagi berganti per batch (lihat catatan di SPINNER_PWM_MAX).
  ...
  // Semua batch melempar pada jarak yang sama (kecepatan penuh); tidak ada
  // lagi batch yang sengaja dijatuhkan lebih dekat ke pusat kolam.
  while (true) {
    maintainNetwork();
    if (backPressed()) {
    ...
    // ---- ANTI-CLOG CHECK ----
    // Chamber belum mendekati kosong, tapi berat tidak berkurang
    // signifikan selama satu jendela waktu -> kemungkinan pakan
    // menyangkut di pintu. Tutup-buka pintu untuk melepaskannya.
    // Jika chamber sudah nol/hampir kosong, loop sudah break di atas
    // sehingga mekanisme ini otomatis tidak berjalan (dibiarkan normal).
    if (millis() - anticlogWindowStart >= ANTICLOG_CHECK_INTERVAL_MS) {
      float dropInWindow = anticlogBaselineGram - gram;
      if (dropInWindow < ANTICLOG_MIN_DROP_G) {
        if (!performAntiClogCycle(batchNo, totalBatches)) {
          lcd.clear(); lcdLine(0,"FEED ABORT"); lcdLine(1,"Closing...");
          stopAllActuators(); delay(1500); lcd.clear();
          setError("FEED_ABORT", "User membatalkan saat anti-clog");
          return false;
        }
        gram = readChamberInstantGram();
        if (gram <= EMPTY_THRESHOLD_G) break;
  ...
  lcd.clear(); lcdLine(0,"BATCH DONE"); lcdLine(1,"Actual:" + fmt1(finalGram) + "g");
  delay(1200); lcd.clear();
  return true;
}
...
bool runFeedingSession(float totalFeedGram, String sessionName) {
  ...
  lcd.clear(); lcdLine(0,"FEEDING DONE"); lcdLine(1,"Total:" + fmt1(totalActualGram) + "g");
...
void showEditFeedingRate() {
  lcdLine(0,"Feeding Rate"); lcdLine(1, fmt1(feedingRatePercent) + " %/hari");
}
...
void showDataFeedInfo() {
  ...
  float df = calculateDailyFeedGram();
  float ps = calculateFeedPerScheduleGram();
  if      (dataFeedInfoPage == 0) { lcdLine(0,"Avg:" + fmt1(sampleAverageGram) + "g"); lcdLine(1,"Ikan:" + String(fishCount)); }
  else if (dataFeedInfoPage == 1) { lcdLine(0,"Biomassa"); lcdLine(1, fmt1(bk) + " kg"); }
  else if (dataFeedInfoPage == 2) { lcdLine(0,"Rate Auto:" + fmt1(feedingRatePercent) + "%"); lcdLine(1,"Hari:" + String(df,0) + "g"); }
...
void handleSampleActive() {
  ...
  if (waitingFishRemove) {
    ...
    lcdLine(1,"SM:" + formatGram1(gram) + "g");
  ...
  if (millis() - lastDisplayUpdate >= DISPLAY_UPDATE_MS) {
    ...
    lcdLine(1, formatGram1(gram) + "g OK=Simpan");
  }
  ...
  if (okPressed()) {
    ...
    lcdLine(1,"Ikan:" + String(currentSampleIndex+1) + " " + fmt1(capturedGram) + "g");
    delay(1200);
    ...
    if (currentSampleIndex >= targetSampleCount - 1) {
      ...
      lcd.clear(); lcdLine(0,"Sampling Done"); lcdLine(1,"AVG:" + fmt1(sampleAverageGram) + "g");
...
void showHistorySelected() {
  ...
  else if (historyMenuIndex == 1) { lcdLine(0, sampleIsManual ? "LAST SAMP(MAN)" : "LAST SAMPLE"); lcdLine(1,"AVG:" + fmt1(lastSampleAverageGram) + "g N:" + String(lastSampleCount)); }
