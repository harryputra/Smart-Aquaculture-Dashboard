...
#include <Update.h>              // OTA: tulis ke slot flash
#include "mbedtls/md.h"          // OTA: sha256 (API generik, stabil di mbedtls 2.x & 3.x)
#include "esp_ota_ops.h"         // OTA: konfirmasi sehat / rollback
...
// =====================================================
// V3.2.1 - SPINNER PWM GANJIL/GENAP (SEBARAN MERATA)
// =====================================================
// Spinner BTS7960 dikontrol via ESP32 LEDC PWM (bukan
// digital HIGH/LOW). Kecepatan spinner TETAP (fixed) per
// batch, bergantian bergantian setiap 2 batch:
//
//   Pasangan 1 (Batch 1 & 2): PWM = 255 (kecepatan PENUH, lempar jauh)
//   Pasangan 2 (Batch 3 & 4): PWM = 175 (kecepatan SEDANG, lempar dekat)
//   (Arah putaran motor selalu berganti CW/CCW setiap 1 batch)
//
// Efek sebaran: batch ganjil melempar pakan lebih jauh
// ke tepian kolam, batch genap menjatuhkan pakan lebih
// dekat ke pusat kolam → distribusi merata tanpa ramp-down.
// =====================================================
...
// =====================================================
// OTA (update firmware jarak jauh) — lihat docs/RENCANA-OTA.md
// =====================================================
const char* FIRMWARE_VERSION = "3.6.0";
// Host dashboard (lewat Cloudflare) untuk unduh .bin & cek manifest self-check.
const char* OTA_API_HOST = "aquaculture.trin-polman.id";
// Validasi sertifikat HTTPS saat unduh: 1 = lewati (integritas TETAP dijamin
// sha256 dari manifest MQTT yang sudah TLS+auth). 0 = validasi → paste OTA_ROOT_CA.
#define OTA_TLS_INSECURE 1
const char* OTA_ROOT_CA = "";                 // PEM root CA bila OTA_TLS_INSECURE 0
...
// MAX = 255 -> batch GANJIL (1,3,5,...): kecepatan penuh, lempar ke tepian ~1.5m
// MIN = 175 -> batch GENAP  (2,4,6,...): kecepatan sedang, jatuh lebih dekat pusat
...
void reconnectMqttIfNeeded() {
  ...
  if (connected) {
    ...
    otaConfirmHealthy();   // MQTT jalan = firmware sehat → batalkan rollback (bila pending)
...
void onMqttMessage(const String& topic, const String& payload, const size_t size) {
  ...
  if (topicStr == topicOta) {
    ...
    if (feedingInProgress) { publishAck("ota", false, "Sedang feeding — OTA ditunda"); }
    otaUrl = url; otaSha256 = sha; otaVersion = ver; otaPending = true;
    publishAck("ota", true, String("OTA diterima: v") + ver);
    return;
  }
...
// =====================================================
// Fungsi ini dipertahankan sebagai referensi, namun
// TIDAK LAGI DIPANGGIL di dispense loop sejak V3.2.1.
// Kini PWM spinner ditentukan TETAP per batch:
//   Batch GANJIL (idx%2==0): SPINNER_PWM_MAX = 255
//   Batch GENAP  (idx%2==1): SPINNER_PWM_MIN = 175
// =====================================================
uint8_t spinnerPWMForDispense(float chamberGramCurrent, float batchTargetGram) {
...
bool runSingleBatch(float targetGram, int batchIndex, int batchNo, int totalBatches, float &actualGram) {
  ...
  //   Batch 1,2 (idx 0,1): (0/2)%2=0 -> PWM = 255, CW / CCW
...
// Konfirmasi firmware baru sehat → batalkan rollback (no-op bila bootloader
// tanpa rollback / bukan boot pasca-OTA).
void otaConfirmHealthy() {
